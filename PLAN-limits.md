...
Migration `0048` therefore only drops the field default, so that *new* users are
...
- Politeness towards TLD operators is bounded by worker concurrency (`-c 4`),
  matching what `check-delegation --concurrency 4` already does today. No
  Celery `rate_limit`, which would be shared between bulk and ad-hoc anyway
  since they are the same task name.
...
New service `celery-delegation`, modelled on `celery-email`:
    ...
    command: celery -A api worker -Q delegation_adhoc,delegation_bulk -c 4
...
- **Backfill** — the `0048` data migration is run against real rows (its
...
- **One per account** (`test_domains.py`) — the second one is a 400 that names
  ...
  works again; the ordinary domain limit still applies alongside.
...
## 11. Files touched
    ...
    api/desecapi/migrations/0048_*.py               new field, backfill, User default
