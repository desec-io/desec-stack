...
``TXT`` record
    ...
    non-printable characters are not accepted even when unicode-escaped).
    Still, you can store any binary data by using DNS-style ``\DDD`` encoding
    for all (or at least non-printable-ASCII) bytes (see RFC 1035 Sec. 3.3.14
    and 5.1). For example, a carriage return (``\r``) can be stored as
    ``\013``. (Note that JSON encoding needs to be applied on top of that, so
    a valid ``records`` field would be ``["\"\\013\""]``.)
