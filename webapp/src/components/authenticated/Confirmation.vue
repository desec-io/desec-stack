<template>
  <v-dialog v-model="value" max-width="500px" persistent @keydown.esc="close">
    <v-card>
      ...
      <v-divider></v-divider>
      <v-alert :value="warning.length" type="warning">{{ warning }}</v-alert>
      <v-alert :value="info.length" type="info">{{ info }}</v-alert>
      <v-card-text>
        <slot></slot>
      </v-card-text>
      <v-card-actions class="pa-3">
        <v-btn color="primary" class="grow" outline @click.native="close">Cancel</v-btn>
        <v-btn color="primary" class="grow" dark depressed @click.native="run">Yes, please</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
