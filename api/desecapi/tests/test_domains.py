...
class IsRegistrableTestCase(DesecTestCase, PublicSuffixMockMixin):
    ...
    def mock(self, global_public_suffixes, local_public_suffixes):
        ...
        class _MockSuffixLists:
            ...
            def __enter__(self):
                self.settings_mocker = test_case.settings(
                    LOCAL_PUBLIC_SUFFIXES=local_public_suffixes
                )
                self.settings_mocker.__enter__()
                self.psl_mocker = test_case.get_psl_context_manager(
                    global_public_suffixes
                )
    ...
    def test_cant_register_ancestors_of_registered_domains(self):
        ...
        with self.mock(
            ...
            self.create_domain(
                owner=user_a, name="_acme-challenge.foobar.public.suffix"
            )
    ...
    def test_can_register_public_suffixes_under_private_domains(self):
        with self.mock(
            global_public_suffixes={"public.suffix"},
            local_public_suffixes={
                "another.public.suffix.private.public.suffix",
                "public.suffix",
            },
        ):
            ...
            self.assertRegistrable(
                "b.another.public.suffix.private.public.suffix", user_c
            )
            self.assertRegistrable(
...
class DomainOwnerTestCase1(DomainOwnerTestCase):
        ...
        for domain in ["non-existent.net", self.other_domain.name, "domain.invalid/"]:
            for name in [domain, f"foo.bar.{domain}"]:
                response = self.client.get(
                    self.reverse("v1:domain-list"), data={"owns_qname": name}
                )
                self.assertStatus(response, status.HTTP_200_OK)
    ...
    def test_retrieve_other_domains(self):
        for domain in self.other_domains:
            response = self.client.get(
                self.reverse("v1:domain-detail", name=domain.name)
            )
            self.assertStatus(response, status.HTTP_404_NOT_FOUND)
...
"""
        ...
        with self.assertRequests(
            ...
            response = self.client.post(
                self.reverse("v1:domain-list"), {"name": name, "zonefile": zonefile}
            )
        self.assertStatus(response, status.HTTP_201_CREATED)
        domain = Domain.objects.get(name=name)
        ...
        self.assertRRsetDB(
            domain, subname="", type_="A", ttl=ttl, rr_contents={"10.1.1.1"}
        )
        self.assertRRsetDB(
            domain, subname="*", type_="A", ttl=ttl, rr_contents={"10.1.1.1"}
        )
        self.assertRRsetDB(
            domain, subname="", type_="TXT", ttl=ttl, rr_contents={'"v=spf1 -all"'}
        )
        self.assertRRsetDB(
            domain,
            subname="_dmarc",
        ...
        self.assertRRsetDB(
            domain, subname="localhost", type_="A", ttl=43200, rr_contents={"127.0.0.1"}
        )
...
"""
        ...
        self.assertEqual(
            ...
            {
                "zonefile": [
                    f"Zonefile includes an SOA record for a name different from {name}."
                ]
            },
        )
...
"""
        ...
        self.assertRRsetDB(
            self.other_domain, subname="inject", type_="CNAME", rr_contents=set()
        )
...
"""
        ...
        self.assertEqual(
            ...
            {
                "zonefile": [
                    "import-me.example/MX: Cannot parse record contents: invalid exchange: \\$url."
                ]
            },
        )
...
"""
        ...
        domain = Domain.objects.get(name=name)
        self.assertRRsetDB(
            domain, subname="", type_="A", ttl=43200, rr_contents={"127.0.0.1"}
        )
...
class AutoDelegationDomainOwnerTests(DomainOwnerTestCase):
    ...
    def test_domain_limit_local_pinned(self):
        """A pinned domain limit takes over from the per-account cap."""
        self.owner.limit_domains = self.owner.domains.count() + 1
        self.owner.save()
        name = self.random_domain_name(self.AUTO_DELEGATION_DOMAINS)
        with self.assertRequests(
            self.requests_desec_domain_creation_auto_delegation(name)
