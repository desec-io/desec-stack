...
class AuthenticatedRRSetTestCase(DomainOwnerTestCase):
    ...
    def test_create_my_rr_sets_mixed_case(self):
        for subname in self.SUBNAMES:
            data = {'records': ['1.2.3.4'], 'ttl': 60, 'type': 'A', 'subname': subname.upper()}
            response = self.client.post_rr_set(self.my_rr_set_domain.name, **data)
            self.assertStatus(response, status.HTTP_409_CONFLICT)
