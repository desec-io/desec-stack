...
class TokenDomainTestCase(DomainOwnerTestCase):
    ...
    def test_domain_permissions(self):
        ...
        # Cannot see other domain while auto_policy=True
        ...
        # Can see other domain when auto_policy=False
        ...
        # Can't delete other domain as long as default policy forbids it
        url = self.reverse("v1:domain-detail", name=domain1.name)
        response = self.client.delete(url)
        self.assertStatus(response, status.HTTP_403_FORBIDDEN)

        # Relax default policy and delete other domain
        default_policy = self.owner.token_with_override.get_policy()
        default_policy.perm_write = True
        default_policy.save()
        with self.assertRequests(self.requests_desec_domain_deletion(domain1)):
            response = self.client.delete(url)
            self.assertStatus(response, status.HTTP_204_NO_CONTENT)
